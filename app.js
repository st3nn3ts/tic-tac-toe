...
// const  mousetable = document.querySelectorAll('.box');
  ...
  // if I use  /**\ (remove mouseout),then mouseover work
...
const table = Array.from(document.getElementsByClassName('box'));
      //struggeling a bit to get safe random pc move
//let free = () => table[i].filter(box => box.innerHTML === "");
//let free = table.filter()box => {box.classList.contains('boxHuman','boxComputer')})
...
const pcmove = free[Math.floor(Math.random()*free.length)].classList.add('boxComputer')
//let pcMove = free[Math.floor(Math.random()*free.length)];
table2[index] = currenP;
played.push(1)
if (currenP === 'o') {
  this.classList.add('boxHuman')
infoTxt.innerHTML = 'pc\'s thinking...',infoTxt.style.color = 'withe'
} else {
  currenP  = 'x'
//  setTimeout(() {
  table[i].classList.add('boxComputer')
//free[Math.floor(Math.random()*free.length)].classList.add('boxComputer')
